...
def bootstrap_index() -> EmbeddingIndex:
    """
    Create or retrieve the application's embedding index configured to use the module INDEX_PATH.
    
    Returns:
        EmbeddingIndex: An EmbeddingIndex instance initialized with INDEX_PATH.
    """
...
def seed_documents() -> list[dict[str, object]]:
    """
    Provide the application's seed documents in a JSON-serializable form.
    
    Returns:
        list[dict[str, object]]: A list of dictionaries, each representing a seed document suitable for JSON serialization and ingestion.
    """
...
def main() -> None:
    """
    Render the Streamlit UI for the Narrative Risk Simulator and orchestrate input validation, document ingestion, retrieval, analysis, and results display.
    
    When invoked, this function builds the app page (title, caption, and three-column layout), collects a draft statement and optional uploaded documents, validates required inputs (non-empty draft and OpenAI API key), parses and combines uploads with the seed corpus, updates the embedding index, retrieves supporting evidence, runs the narrative risk analysis, and presents risk metrics, top reasons, likely narratives, retrieved evidence, and an alternative safer draft in the UI. Errors during upload parsing or analysis are surfaced in the Streamlit interface.
    """
...
def load_documents() -> list[Document]:
    """
    Load and validate the module's seeded documents into Document instances.
    
    Returns:
        list[Document]: A list of validated Document objects representing the seeded documents.
    """
